...
describe("In Words", function() {
  ...
  it("reads teens", function() {
    ...
    return expect(inWords(19)).to.equal('nineteen');
  });
  ...
  it("reads hundreds", function() {
    ...
    return expect(inWords(999)).to.equal('nine hundred ninety nine');
  });
  it("reads thousands", function() {
    ...
    return expect(inWords(32768)).to.equal('thirty two thousand seven hundred sixty eight');
  });
  it("reads millions", function() {
    return expect(inWords(10000001)).to.equal('ten million one');
  });
  it("reads billions", function() {
    return expect(inWords(1234567890)).to.equal('one billion two hundred thirty four million five hundred sixty seven thousand eight hundred ninety');
  });
  return it("reads trillions", function() {
    expect(inWords(1000000000000)).to.equal('one trillion');
    expect(inWords(1000000000001)).to.equal('one trillion one');
    return expect(inWords(1888259040036)).to.equal('one trillion eight hundred eighty eight billion two hundred fifty nine million forty thousand thirty six');
  });
});
